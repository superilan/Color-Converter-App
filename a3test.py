""" 
Unit Test for Assignment A3

This module implements several test cases for a3.  It is incomplete.  You should look 
though this file for places to add tests.

Ilan Klimberg (idk7), Aaron Baruch (amb565)
10/02/2022
""" 
import introcs
import a3


def test_complement():
    """
    Test function complement
    """
    print('Testing complement')
    
    # One test is really good enough here
    comp = a3.complement_rgb(introcs.RGB(250, 0, 71))
    introcs.assert_equals(255-250, comp.red)
    introcs.assert_equals(255-0,   comp.green)
    introcs.assert_equals(255-71,  comp.blue)
    
    # One more for good measure
    comp = a3.complement_rgb(introcs.RGB(128, 64, 255))
    introcs.assert_equals(255-128, comp.red)
    introcs.assert_equals(255-64,  comp.green)
    introcs.assert_equals(255-255, comp.blue)


def test_str5():
    """
    Test function str5
    """
    introcs.assert_equals('130.6',  a3.str5(130.59))
    introcs.assert_equals('130.5',  a3.str5(130.54))
    introcs.assert_equals('100.0',  a3.str5(100))
    introcs.assert_equals('100.6',  a3.str5(100.56))
    introcs.assert_equals('99.57',  a3.str5(99.566))
    introcs.assert_equals('99.99',  a3.str5(99.99))
    introcs.assert_equals('100.0',  a3.str5(99.995))
    introcs.assert_equals('22.00',  a3.str5(21.99575))
    introcs.assert_equals('21.99',  a3.str5(21.994))
    introcs.assert_equals('10.01',  a3.str5(10.013567))
    introcs.assert_equals('10.00',  a3.str5(10.000000005))
    introcs.assert_equals('10.00',  a3.str5(9.9999))
    introcs.assert_equals('9.999',  a3.str5(9.9993))
    introcs.assert_equals('1.355',  a3.str5(1.3546))
    introcs.assert_equals('1.354',  a3.str5(1.3544))
    introcs.assert_equals('0.046',  a3.str5(.0456))
    introcs.assert_equals('0.045',  a3.str5(.0453))
    introcs.assert_equals('0.006',  a3.str5(.0056))
    introcs.assert_equals('0.001',  a3.str5(.0013))
    introcs.assert_equals('0.000',  a3.str5(.0004))
    introcs.assert_equals('0.001',  a3.str5(.0009999))
    introcs.assert_equals('0.000',  a3.str5(1e-9))
    introcs.assert_equals('0.000',  a3.str5(9e-5))
    introcs.assert_equals('0.001',  a3.str5(1e-3))
    

def test_str5_color():
    """
    Test the str5 functions for cmyk and hsv.
    """
    print('Testing str5_cmyk and str5_hsv')
    
    # Tests for str5_cmyk
    # We need to make sure the coordinates round properly
    text = a3.str5_cmyk(introcs.CMYK(98.448, 25.362, 72.8, 1.0))
    introcs.assert_equals('(98.45, 25.36, 72.80, 1.000)',text)
    
    text = a3.str5_cmyk(introcs.CMYK(0.0, 1.5273, 100.0, 57.846))
    introcs.assert_equals('(0.000, 1.527, 100.0, 57.85)',text)
    
    # Tests for str5_hsv (add two)
    text = a3.str5_hsv(introcs.HSV(255.9, 0.5, 0.1))
    introcs.assert_equals('(255.9, 0.500, 0.100)',text)

    #tests for str5_hsv where all the values are the 
    # smallest possible valeus
    text = a3.str5_hsv(introcs.HSV(0, 0, 0))
    introcs.assert_equals('(0.000, 0.000, 0.000)',text)

    #tests for str5_hsv where all the values are the largest 
    # possible values
    text = a3.str5_hsv(introcs.HSV(359.9, 1, 1))
    introcs.assert_equals('(359.9, 1.000, 1.000)',text)

    #tests for str5_hsv where h is the largest possible value
    # , but s and v are the smallest possible values
    text = a3.str5_hsv(introcs.HSV(359.9, 0, 0))
    introcs.assert_equals('(359.9, 0.000, 0.000)',text)

    #tests for str5_hsv where s is the largest possible
    #  value but h and v are the smallest possible values
    text = a3.str5_hsv(introcs.HSV(0, 1, 0))
    introcs.assert_equals('(0.000, 1.000, 0.000)',text)

    #tests for str5_hsv where v is the largest possible 
    # values but h and s are the smallest possible values
    text = a3.str5_hsv(introcs.HSV(0, 0, 1))
    introcs.assert_equals('(0.000, 0.000, 1.000)',text)

    #tests for str5_hsv where none of the values are 
    # the largest or smallest possible values
    text = a3.str5_hsv(introcs.HSV(120.357, 0.5678, 0.23))
    introcs.assert_equals('(120.4, 0.568, 0.230)',text)

    #tests for str5_hsv where one of the values 
    # has scientific notation
    text = a3.str5_hsv(introcs.HSV(359, 1e-20, 0.56324))
    introcs.assert_equals('(359.0, 0.000, 0.563)',text)

    #tests for str5_hsv where one of the values 
    # has too many digits and must be spliced
    text = a3.str5_hsv(introcs.HSV(100.0, .0000001, .5))
    introcs.assert_equals('(100.0, 0.000, 0.500)',text)


def test_rgb_to_cmyk():
    """
    Test translation function rgb_to_cmyk
    """
    print('Testing rgb_to_cmyk')
    
    # The function should guarantee accuracy to three decimal places
    #test for rgb_to_cmyk when r,b, and g are the largest possible values
    rgb = introcs.RGB(255, 255, 255)
    cmyk = a3.rgb_to_cmyk(rgb)
    introcs.assert_equals(0.0, round(cmyk.cyan,3))
    introcs.assert_equals(0.0, round(cmyk.magenta,3))
    introcs.assert_equals(0.0, round(cmyk.yellow,3))
    introcs.assert_equals(0.0, round(cmyk.black,3))

    #test for rgb_to_cmyk when r,b, and g are the
    #  smallest possible values
    rgb = introcs.RGB(0, 0, 0)
    cmyk = a3.rgb_to_cmyk(rgb)
    introcs.assert_equals(0.0, round(cmyk.cyan,3))
    introcs.assert_equals(0.0, round(cmyk.magenta,3))
    introcs.assert_equals(0.0, round(cmyk.yellow,3))
    introcs.assert_equals(100.0, round(cmyk.black,3))

    #test for rgb_to_cmyk when r,b, and g are neither 
    # the smallest nor largest possible values
    rgb = introcs.RGB(217, 43, 164)
    cmyk = a3.rgb_to_cmyk(rgb)
    introcs.assert_equals(0.0, round(cmyk.cyan,3))
    introcs.assert_equals(80.184, round(cmyk.magenta,3))
    introcs.assert_equals(24.424, round(cmyk.yellow,3))
    introcs.assert_equals(14.902, round(cmyk.black,3))


def test_cmyk_to_rgb(): 
    """
    Test translation function cmyk_to_rgb
    """
    print('Testing cmyk_to_rgb')
    #testing cmyk_to_rgb when all the parameters are the 
    # smallest possible values
    cmyk = introcs.CMYK(0, 0, 0, 0)
    rgb = a3.cmyk_to_rgb(cmyk)
    introcs.assert_equals(255, rgb.red)
    introcs.assert_equals(255, rgb.green)
    introcs.assert_equals(255, rgb.blue)

    #testing cmyk_to_rgb when all the parameters are the 
    # largest possible values
    cmyk = introcs.CMYK(100, 100, 100, 100)
    rgb = a3.cmyk_to_rgb(cmyk)
    introcs.assert_equals(0, rgb.red)
    introcs.assert_equals(0, rgb.green)
    introcs.assert_equals(0, rgb.blue)

    #testing cmyk_to_rgb when only k is the largest possible value
    cmyk = introcs.CMYK(90, 40, 2, 100)
    rgb = a3.cmyk_to_rgb(cmyk)
    introcs.assert_equals(0, rgb.red)
    introcs.assert_equals(0, rgb.green)
    introcs.assert_equals(0, rgb.blue)

    #testing cmyk_to_rgb when only k is the largest possible 
    #value and c, m, and y are the smallest possible values
    cmyk = introcs.CMYK(0, 0, 0, 100)
    rgb = a3.cmyk_to_rgb(cmyk)
    introcs.assert_equals(0, rgb.red)
    introcs.assert_equals(0, rgb.green)
    introcs.assert_equals(0, rgb.blue)

    #testing cmyk_to_rgb when only k is the smallest possible value
    cmyk = introcs.CMYK(90, 40, 2, 0)
    rgb = a3.cmyk_to_rgb(cmyk)
    introcs.assert_equals(25, rgb.red)
    introcs.assert_equals(153, rgb.green)
    introcs.assert_equals(250, rgb.blue)

    #testing cmyk_to_rgb for accuracy to 3 decimal places
    cmyk = introcs.CMYK(53.5, 89, 23.987765, 3)
    rgb = a3.cmyk_to_rgb(cmyk)
    introcs.assert_equals(115, rgb.red)
    introcs.assert_equals(27, rgb.green)
    introcs.assert_equals(188, rgb.blue)

    #testing cmyk_to_rgb for accuracy to 3 decimal places
    cmyk = introcs.CMYK(60.0, 20.0, 30.454, 89)
    rgb = a3.cmyk_to_rgb(cmyk)
    introcs.assert_equals(11, rgb.red)
    introcs.assert_equals(22, rgb.green)
    introcs.assert_equals(20, rgb.blue)

    #testing cmyk_to_rgb for when k is a decimal
    cmyk = introcs.CMYK(90.343, 40.535, 2.3454, 0.34532)
    rgb = a3.cmyk_to_rgb(cmyk)
    introcs.assert_equals(25, rgb.red)
    introcs.assert_equals(151, rgb.green)
    introcs.assert_equals(248, rgb.blue)


def test_rgb_to_hsv():
    """
    Test translation function rgb_to_hsv
    """
    print('Testing rgb_to_hsv')

    rgb = introcs.RGB(249, 28, 122)
    hsv = a3.rgb_to_hsv(rgb)
    introcs.assert_equals(334.48, round(hsv.hue, 3))
    introcs.assert_equals(0.888, round(hsv.saturation, 3))
    introcs.assert_equals(0.976, round(hsv.value, 3))

    #max_rgb == min_rgb
    rgb = introcs.RGB(255, 255, 255)
    hsv = a3.rgb_to_hsv(rgb)
    introcs.assert_equals(0.0, round(hsv.hue,3))
    introcs.assert_equals(0.0, round(hsv.saturation,3))
    introcs.assert_equals(1.0, round(hsv.value,3))
    
    #max_rgb == r and g > b
    rgb = introcs.RGB(255, 100, 50)
    hsv = a3.rgb_to_hsv(rgb)
    introcs.assert_equals(14.634, round(hsv.hue,3))
    introcs.assert_equals(0.804, round(hsv.saturation,3))
    introcs.assert_equals(1.0, round(hsv.value,3))

    #max_rgb == r and g == b
    rgb = introcs.RGB(255, 100, 100)
    hsv = a3.rgb_to_hsv(rgb)
    introcs.assert_equals(0.0, round(hsv.hue,3))
    introcs.assert_equals(0.608, round(hsv.saturation,3))
    introcs.assert_equals(1.0, round(hsv.value,3))

    #max_rgb == r and g < b
    rgb = introcs.RGB(255, 100, 150)
    hsv = a3.rgb_to_hsv(rgb)
    introcs.assert_equals(340.645, round(hsv.hue,3))
    introcs.assert_equals(0.608, round(hsv.saturation,3))
    introcs.assert_equals(1.0, round(hsv.value,3))

    #max_rgb == g
    rgb = introcs.RGB(0, 255, 0)
    hsv = a3.rgb_to_hsv(rgb)
    introcs.assert_equals(120.0, round(hsv.hue,3))
    introcs.assert_equals(1.0, round(hsv.saturation,3))
    introcs.assert_equals(1.0, round(hsv.value,3))

    #max_rgb == b
    rgb = introcs.RGB(0, 0, 255)
    hsv = a3.rgb_to_hsv(rgb)
    introcs.assert_equals(240.0, round(hsv.hue,3))
    introcs.assert_equals(1.0, round(hsv.saturation,3))
    introcs.assert_equals(1.0, round(hsv.value,3))

    #max_rgb == 0
    rgb = introcs.RGB(0, 0, 0)
    hsv = a3.rgb_to_hsv(rgb)
    introcs.assert_equals(0.0, round(hsv.hue,3))
    introcs.assert_equals(0.0, round(hsv.saturation,3))
    introcs.assert_equals(0.0, round(hsv.value,3))


def test_hsv_to_rgb(): 
    """
    Test translation function hsv_to_rgb
    """
    print('Testing hsv_to_rgb')
    #testing hsv_to_rgb when the values returned are not zero
    hsv = introcs.HSV(250.0, 0.522, 0.253)
    rgb = a3.hsv_to_rgb(hsv)
    introcs.assert_equals(36, rgb.red)
    introcs.assert_equals(31, rgb.green)
    introcs.assert_equals(65, rgb.blue)

    #testing hsv_to_rgb when h, s, and v are at 
    # their maximum value
    hsv = introcs.HSV(359.9, 1, 1)
    rgb = a3.hsv_to_rgb(hsv)
    introcs.assert_equals(255, rgb.red)
    introcs.assert_equals(0, rgb.green)
    introcs.assert_equals(0, rgb.blue)

    #testing hsv_to_rgb where G is at its maximum value
    hsv = introcs.HSV(120.0, 1, 1)
    rgb = a3.hsv_to_rgb(hsv)
    introcs.assert_equals(0, rgb.red)
    introcs.assert_equals(255, rgb.green)
    introcs.assert_equals(0, rgb.blue)

    #testing hsv_to_rgb where B is at its maximum value
    hsv = introcs.HSV(240.0, 1, 1)
    rgb = a3.hsv_to_rgb(hsv)
    introcs.assert_equals(0, rgb.red)
    introcs.assert_equals(0, rgb.green)
    introcs.assert_equals(255, rgb.blue)

    #testing hsv_to_rgb where R, B, and G are equal
    hsv = introcs.HSV(0, 0, 0.333)
    rgb = a3.hsv_to_rgb(hsv)
    introcs.assert_equals(85, rgb.red)
    introcs.assert_equals(85, rgb.green)
    introcs.assert_equals(85, rgb.blue)

    #testing hsv_to_rgb when H subset i is 0
    hsv = introcs.HSV(0, 0.4, 0.3)
    rgb = a3.hsv_to_rgb(hsv)
    introcs.assert_equals(76, rgb.red)
    introcs.assert_equals(46, rgb.green)
    introcs.assert_equals(46, rgb.blue)

    #testing hsv_to_rgb when H subset i is 1
    hsv = introcs.HSV(60, 0.4, 0.3)
    rgb = a3.hsv_to_rgb(hsv)
    introcs.assert_equals(76, rgb.red)
    introcs.assert_equals(76, rgb.green)
    introcs.assert_equals(46, rgb.blue)

    #testing hsv_to_rgb when H subset i is 2
    hsv = introcs.HSV(120, 0.4, 0.3)
    rgb = a3.hsv_to_rgb(hsv)
    introcs.assert_equals(46, rgb.red)
    introcs.assert_equals(76, rgb.green)
    introcs.assert_equals(46, rgb.blue)

    #testing hsv_to_rgb when H subset i is 3
    hsv = introcs.HSV(180, 0.4, 0.3)
    rgb = a3.hsv_to_rgb(hsv)
    introcs.assert_equals(46, rgb.red)
    introcs.assert_equals(76, rgb.green)
    introcs.assert_equals(76, rgb.blue)

    #testing hsv_to_rgb when H subset i is 4
    hsv = introcs.HSV(240, 0.4, 0.3)
    rgb = a3.hsv_to_rgb(hsv)
    introcs.assert_equals(46, rgb.red)
    introcs.assert_equals(46, rgb.green)
    introcs.assert_equals(76, rgb.blue)

    #testing hsv_to_rgb when H subset i is 5
    hsv = introcs.HSV(300, 0.4, 0.3)
    rgb = a3.hsv_to_rgb(hsv)
    introcs.assert_equals(76, rgb.red)
    introcs.assert_equals(46, rgb.green)
    introcs.assert_equals(76, rgb.blue)


def test_contrast_value():
    """
    Test translation function contrast_value
    """
    print('Testing contrast_value')
    
    # contrast == -1.0 (extreme)
    result = a3.contrast_value(0.0,-1.0)
    introcs.assert_floats_equal(0.5,result)
    
    result = a3.contrast_value(1.0,-1.0)
    introcs.assert_floats_equal(0.5,result)
    
    # contrast < 0, bottom part of sawtooth
    result = a3.contrast_value(0.1,-0.5)
    introcs.assert_floats_equal(0.3,result)
    
    # contrast < 0, middle of sawtooth
    result = a3.contrast_value(0.4,-0.4)
    introcs.assert_floats_equal(0.4571429,result)
    
    # contrast < 0, upper part of sawtooth
    result = a3.contrast_value(0.9,-0.3)
    introcs.assert_floats_equal(0.8142857,result)
    
    # contrast == 0.0, bottom part of sawtooth
    result = a3.contrast_value(0.1,0.0)
    introcs.assert_floats_equal(0.1,result)
    
    # contrast == 0, middle of sawtooth
    result = a3.contrast_value(0.6,0.0)
    introcs.assert_floats_equal(0.6,result)
    
    # contrast == 0.0, upper part of sawtooth
    result = a3.contrast_value(0.9,0.0)
    introcs.assert_floats_equal(0.9,result)
    
    # contrast > 0, bottom part of sawtooth
    result = a3.contrast_value(0.1,0.3)
    introcs.assert_floats_equal(0.05384615,result)
    
    # contrast > 0, middle of sawtooth
    result = a3.contrast_value(0.4,0.5)
    introcs.assert_floats_equal(0.2,result)
    
    # contrast > 0, upper part of sawtooth
    result = a3.contrast_value(0.9,0.4)
    introcs.assert_floats_equal(0.95714286,result)
    
    # contrast == 1.0 (extreme)
    result = a3.contrast_value(0.2,1.0)
    introcs.assert_floats_equal(0.0,result)
    
    result = a3.contrast_value(0.6,1.0)
    introcs.assert_floats_equal(1.0,result)


def test_contrast_rgb(): 
    """
    Test translation function contrast_value
    """
    print('Testing contrast_rgb')
    
    # Negative contrast
    rgb = introcs.RGB(240, 15, 118)
    hsv = a3.contrast_rgb(rgb,-0.4)
    introcs.assert_equals(220, rgb.red)
    introcs.assert_equals(35,  rgb.green)
    introcs.assert_equals(123, rgb.blue)

    #Positive contrast
    rgb = introcs.RGB(230, 230, 230)
    hsv = a3.contrast_rgb(rgb,0.4)
    introcs.assert_equals(244, rgb.red)
    introcs.assert_equals(244,  rgb.green)
    introcs.assert_equals(244, rgb.blue)

    rgb = introcs.RGB(102, 102, 102)
    hsv = a3.contrast_rgb(rgb,0.5)
    introcs.assert_equals(51, rgb.red)
    introcs.assert_equals(51,  rgb.green)
    introcs.assert_equals(51, rgb.blue)

    #Contrast == 0.0
    rgb = introcs.RGB(0, 0, 0)
    hsv = a3.contrast_rgb(rgb,0.0)
    introcs.assert_equals(0, rgb.red)
    introcs.assert_equals(0,  rgb.green)
    introcs.assert_equals(0, rgb.blue)

    rgb = introcs.RGB(230, 26, 230)
    hsv = a3.contrast_rgb(rgb,0.0)
    introcs.assert_equals(230, rgb.red)
    introcs.assert_equals(26,  rgb.green)
    introcs.assert_equals(230, rgb.blue)

    #Contrast == 1.0
    rgb = introcs.RGB(51, 153, 51)
    hsv = a3.contrast_rgb(rgb,1.0)
    introcs.assert_equals(0, rgb.red)
    introcs.assert_equals(255,  rgb.green)
    introcs.assert_equals(0, rgb.blue)

    #Contrast == -1.0
    rgb = introcs.RGB(0, 255, 0)
    hsv = a3.contrast_rgb(rgb,-1.0)
    introcs.assert_equals(128, rgb.red)
    introcs.assert_equals(128,  rgb.green)
    introcs.assert_equals(128, rgb.blue)


# Script Code
# THIS PREVENTS THE TESTS RUNNING ON IMPORT
if __name__ == '__main__':
    test_complement()
    test_str5()
    test_str5_color()
    test_rgb_to_cmyk()
    test_cmyk_to_rgb()
    test_rgb_to_hsv()
    test_hsv_to_rgb()
    test_contrast_value()
    test_contrast_rgb()
    print('Module a3 passed all tests.')
